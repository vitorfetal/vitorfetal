<div align="center">

# 👨🏻‍💻 Vitor Fetal

**`Desenvolvedor Backend Java`**

Me chamo Vitor Fetal, sou desenvolvedor backend focado em Java, Spring Boot e arquitetura de software. Atualmente estou construindo sistemas escaláveis aplicando conceitos de DDD (Domain-Driven Design) e Clean Architecture.

Sou apaixonado por engenharia de software e estou constantemente evoluindo minhas habilidades em backend, modelagem de domínio e boas práticas de desenvolvimento.

</div>

---

<h3 align="center">🤖 Linguagens e Tecnologias</h3>

<p align="center">
  <img align="left" alt="Java" title="Java" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/java/java-original.svg"/>
  
  <img align="left" alt="Spring Boot" title="Spring Boot" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/spring/spring-original.svg"/>
  
  <img align="left" alt="C" title="C" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/c/c-original.svg"/>
  
  <img align="left" alt="C#" title="C#" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/csharp/csharp-original.svg"/>
  
  <img align="left" alt="MySQL" title="MySQL" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mysql/mysql-original.svg"/>
  
  <img align="left" alt="PostgreSQL" title="PostgreSQL" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/postgresql/postgresql-original.svg"/>
  
  <img align="left" alt="Docker" title="Docker" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/docker/docker-original.svg"/>
  
  <img align="left" alt="Git" title="Git" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original.svg"/>
  
  <img align="left" alt="OpenAPI" title="OpenAPI" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/openapi/openapi-original.svg"/>
  
  <img align="left" alt="Swagger" title="Swagger" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/swagger/swagger-original.svg"/>
  
  <img align="left" alt="IntelliJ" title="IntelliJ IDEA" width="30px" style="padding-right: 10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/intellij/intellij-original.svg"/>
</p>

<br/>
<br/>

---

### 🧠 Arquitetura e Boas Práticas

- Domain-Driven Design (DDD)
- Clean Architecture
- Arquitetura em camadas
- SOLID Principles
- Design de APIs REST

---

### 🚀 Projeto em Destaque

**PrismaHR (System RH)**  
Sistema de gestão de Recursos Humanos focado em recrutamento, gestão de candidatos e processos seletivos.

- Modelagem de domínio (DDD)
- Arquitetura modular
- Backend escalável em Java
- Foco em boas práticas de engenharia de software

---

### 🎯 Objetivo Atual

- Evoluir como Backend Engineer
- Aprofundar em Spring Boot e arquitetura de sistemas
- Construir sistemas reais e escaláveis
- Preparação para oportunidades internacionais

---

### 📊 Estatísticas

<div align="center">

<img src="https://streak-stats.demolab.com?user=vitorfetal&theme=tokyonight" />

<img src="https://github-readme-activity-graph.vercel.app/graph?username=vitorfetal&theme=tokyo-night" />

</div>
